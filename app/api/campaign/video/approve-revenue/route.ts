...
export async function PUT(req: NextRequest) {
  try {
    ...
    if(video.revenueStatus == "Approved") {
      return NextResponse.json({ message: 'Video revenue already approved!' }, { status: 200 });
    }
    ...
    await videoRef.update({ revenueStatus: "Approved" });

    return NextResponse.json({ message: 'Video revenue approved successfully!' }, { status: 200 });
  } catch (error) {
    console.error('Error approving the video revenue:', error);
